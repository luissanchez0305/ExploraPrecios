                   ...
                   { %>
                        <ul>
                         <% foreach (var subcategory in category.subCategories)
                            { %>
                                <li><a href="/?catlev=2&id=<%= subcategory.subCategoryId %>"><%= subcategory.subCategoryTitle %></a>
                                <% if (subcategory.productTypes.Count > 0)
                                   { %>
                                        <ul>
                                        <% foreach (var producttype in subcategory.productTypes)
                                           { %>
                                                <li><a href="/?catlev=3&id=<%= producttype.productTypeId %>"><%= producttype.productTypeTitle %></a></li>
                                        <% } %>
                                        </ul>
                                <%} %></li>
                         <% } %>
       ...
       {%>
        ...
        <ul class="suckertree1">
            <% foreach (var filterItem in filter.items.Where(filterItem => filterItem.name.Length > 0))
               { %>
                <li><a class="filter" href="<%= filterItem.url %>"><%= filterItem.name%></a></li>
            <%} %>
        </ul>
        </div>
    <% } %>
