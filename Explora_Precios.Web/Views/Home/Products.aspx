...
<asp:Content ID="ProductContent" ContentPlaceHolderID="MainContent" runat="server">
	...
	<script type="text/javascript">
		...
		function enlarge(ctr, add) {
			var gridItem = $(ctr);            
			
			if(gridItem.hasClass('gridItem'))
			{
				//grid
				gridItem.toggleClass('gridItem_on', add);
			}
			else
			{
				//list
				gridItem.toggleClass('gridItem_v_on', add);
			}

//			if (add)
//				gridItem.css('border-width', '2px');
//			else
//				gridItem.css('border-width', '1px');
		}
		function changeDisplay(ctr)
		{
			var source = $(ctr);
			$('#CurrentProductDisplay').val(source.attr('class'));
			switchDisplay(source.attr('class'));
		}
